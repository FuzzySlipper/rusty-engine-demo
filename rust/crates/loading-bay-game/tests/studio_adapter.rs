...
fn open_uses_engine_owners_and_returns_canonical_projection_and_voxel_readouts() {
    ...
    assert_eq!(response["project"]["inspections"]["scene"]["nodeCount"], 16);
    assert_eq!(
...
fn project_creation_and_save_as_publish_admitted_canonical_projects() {
    ...
    assert_eq!(created["project"]["identity"]["currentSchemaVersion"], 15);
    ...
    assert_eq!(created_document.source_schema_version, 15);
