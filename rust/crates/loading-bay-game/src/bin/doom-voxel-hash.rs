...
fn main() {
    ...
    let path = args
        .get(1)
        .expect("usage: doom-voxel-hash <path-to-voxel-json>");
    let path_buf = PathBuf::from(path);
    let input = fs::read_to_string(&path_buf)
        .unwrap_or_else(|e| panic!("read {}: {e}", path_buf.display()));
    let asset = decode_voxel_asset(&input).unwrap_or_else(|e| {
        ...
        let raw: serde_json::Value = serde_json::from_str(&input).expect("json parse");
    ...
    let fixed =
        with_computed_content_hash(asset).unwrap_or_else(|e| panic!("recompute failed: {e}"));
    ...
    println!(
        "fixed {} voxel_data_hash={} content_hash={}",
        path_buf.display(),
        fixed.voxel_data_hash,
        fixed.content_hash
    );
}
