...
| P6806-2 | At L2 and L4, seven repeated `NavigationUnreachable`/`NavigationBlocked` records displaced useful player feedback in the visible committed-facts panel. These are internal enemy navigation diagnostics, not actionable play information. | Angular combat-log projection only. `Navigation*` and `EnemyPostureChanged` stay in the authoritative snapshot/diagnostics but are excluded from the seven-row player-facing log. | The exact pre-fix L2 image shows the flood. Rebuilt L4/secret captures show only bounded player-facing facts while the same Rust route and enemy state continue. |
