...
> Historical browser-renderer baseline: task #6703 moved rendered product authority to the
> Engine-owned Rust adapter. [`design.md`](design.md) owns the current boundary; browser renderer
> references below document predecessor behavior and are not implementation guidance.
...
| Inventory quantities, item containment, equipment, tracks/effects   | Engine gameplay-mechanics owners through the adjacent facade | Loading Bay policy and projections |
...
| Retained render resources, surface lifecycle, timing, backend       | Rusty Engine application host through the complete facade    | Loading Bay content and UI         |
...
cooldown, damage, target selection, impact-once, and expiry policy. The adjacent Engine facade's
