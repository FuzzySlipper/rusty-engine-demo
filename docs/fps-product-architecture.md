...
| Content              | TypeScript composes immutable schema-13 project JSON. Rust decodes, validates, admits, and materializes it. Current meanings include stable item definitions, authored starting inventory, and non-solid pickup objects; Rust alone owns live stacks, ordering, capacity, equipped identity, pickup availability, and atomic world-consume/item-grant transactions. The legacy attached weapon counter remains isolated until its inventory-ammo cutover in #6222. | Key progression, enemy archetypes, and the complete original route remain downstream game content and semantics in #6223–#6229.                         |
