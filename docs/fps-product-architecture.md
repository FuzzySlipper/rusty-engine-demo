...
| Content              | TypeScript composes immutable schema-14 project JSON. Weapon item definitions own attack mode, damage, range, cadence, ammunition item/cost, muzzle offset, and presentation identity. Rust alone owns live stacks, authored numeric slots, equipped identity, per-weapon cooldown, pickup availability, atomic starter-ammunition grants, and ammunition consumption. The schema-6 attached weapon is migration input only and cannot enter a current session. | Key progression, enemy archetypes, and the complete original route remain downstream game content and semantics in #6223–#6229.                          |
| Live persistence     | `GameSnapshot` schema 13 round-trips entities, collision, doors, switches, enemies, health, encounters, navigation, controllers, item definitions, inventory quantities, equipped weapon, authored slots, per-weapon cooldowns, pickup state, schedules, and tick. `ProjectStore` separately persists authored project content.                                                                                                                                 | #6230 adds save slots/checkpoints without persisting connection or presentation state.                                                                   |
...
4. **Combat:** validate cooldown, ownership, ammunition, aim, occlusion, damage, armor, death, and
   deterministic drops.
5. **Interactions and pickups:** apply item grants, switches, doors, secrets, and exit transitions
   transactionally.
6. **Scheduled consequences:** drain due, bounded game-specific consequences such as door close.
7. **Projection and fact drain:** produce one immutable dynamic projection plus ordered facts/cues
   for this tick.
