...
| Content              | TypeScript composes immutable schema-12 project JSON. Rust decodes, validates, admits, and materializes it. Current meanings include stable item definitions plus authored starting inventory; Rust alone owns live stacks, ordering, capacity, and equipped identity. The legacy attached weapon counter remains isolated until its inventory-ammo cutover in #6222. | Pickups, key progression, enemy archetypes, and the complete original route remain downstream game content and semantics in #6221–#6229.                 |
